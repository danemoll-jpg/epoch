...
| # | Item | Status | Verified |
|---|---|---|---|
| 0 | Docs commit | Done: `fa74bb6` (CLAUDE.md, TODO.md only), then re-read both. Nothing from the Round 17 report was dropped. | n/a |
| 1 | The auto-selected next unit brought into view | Done. Whenever the game picks the next unit itself (after a move, Fortify, Found City, boarding, going ashore, an airlift, an attack, "Move … here"), the map now **pans smoothly to it only if it isn't comfortably in view**: off screen, within half a tile of an edge, or under the top bar, ☰, the minimap, the unit panel, the city panel, or End Turn. It then sits in the middle of the uncovered part (below the top bar, above the unit panel, left of the landscape city panel / above the portrait sheet). About a quarter second (faster with Animation speed: Fast; instant with the device's reduced-motion setting); a drag or pinch stops it. A unit already in view: the map doesn't move. **Next Unit** still centers as before. Closing the city panel by hand (✕ or Esc) also brings the selected unit back into view. **The two checks:** *boarding* was not a real bug: boarding (a step onto a ship, or ⚓ Board in port) already ends the unit's moves, so the next unit came, but off screen (the item 1 bug). I made the rule sturdier anyway: after a move the next unit comes when the selected one is no longer *ready* (no moves, fortified, or now riding a ship), not only at 0 moves. *Stopping short* with moves left (the way on turned out blocked, or the next tile costs more than it has left) keeps the unit selected (as intended) and now **says why** in a toast ("Legion stopped short: the way on is blocked"). | Unit-tested (`tileComfortablyVisible`/`centerInRect`: middle, edges, under the top bar and the unit panel, the pan target; boarding at sea and in port ends the moves; a stopped-short move keeps them). Scenario outcome. Preview-verified on desktop in iPad landscape emulation (1180×820): `next-unit-in-view`: one tap boarded, the Galley (in view) was selected with no pan; after the Galley sailed, the Horseman 8 tiles east was selected and the map panned to center it above the unit panel. |
| 2 | Wake, the Units list, How to Play | Done. **Wake:** with a fortified unit selected the Fortify button reads **Wake** (gold); ships, aircraft, Helicopters and Missionaries: "Stay" → **Wake** too (it used to read "Fortified"/"Staying", disabled). A new `wake` action (through `applyAction`): the unit loses the fortified bonus, stays selected, and Next Unit offers it again. Its moves are untouched, so a unit fortified earlier is ready at once ("Archer is awake and ready to move"), and one fortified *this* turn (Fortify uses up the turn) says "…is awake: it can move next turn". **Reaching fortified units:** tapping a fortified unit on the map, in the city panel's unit list, or in the stack list selects it (checked: all three already did). New **☰ → Units** (key U on a PC): every unit you have, with filters **All / Ready / Fortified / Aboard** (each with its count), each row with its icon, ★, status (🛡 fortified / staying put / ⚓ aboard / ready / done this turn) and where it is ("in Babylon", "3 tiles from Ur"); tapping one closes the menu, selects it, and centers the map on it. I put it in ☰ rather than the top bar, which is already full on a portrait iPad. **How to Play** (the "Moving and founding cities" page): how to wake a fortified unit (select it by tapping it, from its city, or from ☰ → Units, then Wake), plus the new tap rule and the auto-select pan. No state change, so no save-version bump. | Unit-tested (wake: ready again, fortified this turn, a ship, not fortified, not your turn; the list's statuses, filters, counts, order, and "where"; an aircraft on a Carrier isn't "aboard"). Scenario outcome. Preview-verified in landscape emulation (`fortified-units`: ☰ → Units → Fortified listed the Warrior, Archer and Galley; tapping the Archer selected and centered it with Wake; Wake gave the toast and put it in Next Unit; the city panel's list selected the fortified Warrior with Wake; the Galley's button read Wake). |
| 3 | Tapping your own units from a distance | Done, as decided. New rule in `src/ui/tap.ts` (comment updated): with a unit selected, **tapping a tile holding your own units selects one there** (a ship before its cargo) **unless the selected unit is next to that tile** (any of the 8), where one tap still moves in or boards, as before. When the selected unit could get there, the stack list offers **"Move Legion here (3 turns)"** or, for a land unit and a ship at sea, **"⚓ Board the Galley (Legion, 2 turns)"**; it moves the unit as far as it gets this turn and, for a longer trip, says "Legion is on its way: next turn, select it and tap the same spot again to carry on". So forming an army from a distance is: tap the stack, then Move Legion here. Your own city keeps Round 17's rule; enemies (attack), empty tiles, and aircraft (a Carrier in range is still a rebase) are unchanged. No press-and-hold. How to Play updated. | Unit-tested (far/next/diagonal, can't get there, a ship far off vs next door, cargo picked last, a board offer the path can reach and follow, no moves left, aircraft unchanged, your city unchanged). Scenario outcome. Preview-verified in iPad portrait emulation (820×1180) (`tap-own-unit`: the Galley was selected with the Board offer and the Legion didn't move; the stack was selected with "Move Legion here (3 turns)"; the button moved the Legion, toasted, and the game selected the Warrior; one tap on the stack next door moved it in). |
| 4 | Wrap-up | Done. Scenarios **`next-unit-in-view`**, **`fortified-units`**, **`tap-own-unit`**, each with a note and an outcome in `tests/scenarios.test.ts` (the notes' turn counts are checked against `pathTurns`). `tests/round18.test.ts` (22 tests). **984 pass, 6 skipped** (the emulator rules tests, as before), `pace.test.ts` included. `npm run lint` and `npm run build` clean (check-dist OK; first load 699 KB, 237 KB gzipped). Preview-verified on desktop, in landscape (1180×820) and portrait (820×1180) iPad emulation, no console errors. | `npm test`, `npm run lint`, `npm run build`, preview. |
...
- The "Move … here" offer sits in the unit panel's stack list of the unit you tapped (it's that panel you're looking at), and goes away as soon as you select something else.
- Units list: in ☰, not the top bar (no room on a portrait iPad).
...
* **Round 19 — The big playtest update: victory warnings, era and wonder cards, the news log, obsolete units and upgrades, spies (plus Modern Infantry and the Drone), more wars, culture borders and referendums, full-screen leader scenes, and a balance pass — done, pushed and live as 0.19.0 (2026-09-26).** Dan picked the Spy icon (A) and approved the leader-scene crops. Balance only partly on target: the leftovers are Round 20. The agent's progress notes and report follow, moved from Current Objective.
...
| 6 | Leader scenes | Done (Part F): all 12 originals present; masters in `docs/portraits-full-master/`, 1200 px WebP (1,702 KB, cached on first use, not up front). Used for first contact, Diplomacy, demands/offers, war both ways, peace, and met rivals' victory warnings. **Dan approved the crops (2026-09-26).** | Unit-tested (face clear of the words at 5 screen shapes); scenario `leader-scenes`; preview (portrait, landscape). |
...
- **The later balance round — part B of Round 19, then Round 20:** from Round 15's leftovers: North Korea (1 win in 54), Russia, and the Franks under their share (conquerors struggle to finish); no domination on Huge or Epic; Huge 45% economic; Legendary 55% technology; Small games can run long (to t266). Use `npm run sim -- matrix` with more games per row (20 is noisy).
